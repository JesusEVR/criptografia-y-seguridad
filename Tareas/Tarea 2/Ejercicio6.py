...
if __name__ == "__main__":
    ...
    message = b"noche697"
    key_text = "data7Qa="
    key = int.from_bytes(key_text.encode("ascii"), "big")

    print("\n>>> Cifrando")
    print(f"- Mensaje original: {message.decode('ascii')}")
    print(f"- Clave usada: {key_text}")
    cipher_bytes = des_encrypt(message, key)
    cipher_base64 = base64.b64encode(cipher_bytes).decode("ascii")
    print(f"- Texto cifrado en Base64: {cipher_base64}")
    
    print("\n>>> Descifrando")
    bytes_to_decrypt = base64.b64decode(cipher_base64)
    decrypted_bytes = des_decrypt(bytes_to_decrypt, key)
    print(f"- Texto descifrado: {decrypted_bytes.decode('ascii')}\n")